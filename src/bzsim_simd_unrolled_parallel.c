...
struct WorkQueue
{
    cnd_t ordersRefillCnd;
    mtx_t ordersRefillMtx;

    WorkOrder *orders;
    _Atomic S32 nextOrderIndex;
    _Atomic S32 unprocessedOrdersCount;

    _Atomic S32 activeThreadsCount;
    _Atomic B32 quit;
};
...
internal B32
bzsim_process_next_order(WorkQueue *queue)
{
    S32 index = queue->nextOrderIndex--;
    if (index < 0) {
        return False;
    }
    WorkOrder *order = queue->orders + index;
    BZSim *bz = order->bz;
    ...
    U32 *pixels = order->pixels;

    S32 width = bz->width;

    S32 xStart = order->xStart;
    S32 xEnd = order->xEnd;
    S32 yStart = order->yStart;
    S32 yEnd = order->yEnd;
    ...
    queue->unprocessedOrdersCount--;
    return True;
}

internal S32
bzsim_worker_thread(void *arg)
{
    WorkQueue *queue = (WorkQueue *)arg;
    queue->activeThreadsCount++;
    while (!queue->quit) {
        if (!bzsim_process_next_order(queue)) {
            mtx_lock(&queue->ordersRefillMtx);
            cnd_wait(&queue->ordersRefillCnd, &queue->ordersRefillMtx);
            mtx_unlock(&queue->ordersRefillMtx);
        }
    }
    queue->activeThreadsCount--;
    return 1;
...
bzsim_init(S32 width, S32 height, F32 alpha, F32 betta, F32 gamma)
{
    ...
    queue->nextOrderIndex = -1;
    queue->orders = calloc(height, sizeof(WorkOrder));
    mtx_init(&queue->ordersRefillMtx, mtx_plain);
    cnd_init(&queue->ordersRefillCnd);

    S32 threadsCount = get_nprocs() - 1;
    for (S32 i = 0; i < threadsCount; i++) {
        thrd_t thrd;
        thrd_create(&thrd, bzsim_worker_thread, queue);
        thrd_detach(thrd);
    }
...
bzsim_update(BZSim *bz, U32 *pixels)
{
    ...
    WorkQueue *queue = bz->queue;

    S32 nextOrderIndex = 0;
    S32 unprocessedOrdersCount = 0;
    for (S32 y = 1; y <= height; y++) {
        nextOrderIndex = unprocessedOrdersCount++;
        WorkOrder *order = queue->orders + nextOrderIndex;
        order->pixels = pixels;
        order->bz = bz;
        order->xStart = 1;
        order->xEnd = width;
    ...
    mtx_lock(&queue->ordersRefillMtx);
    queue->nextOrderIndex = nextOrderIndex;
    queue->unprocessedOrdersCount = unprocessedOrdersCount;
    cnd_broadcast(&queue->ordersRefillCnd);
    mtx_unlock(&queue->ordersRefillMtx);

    while (queue->unprocessedOrdersCount > 0) {
        bzsim_process_next_order(queue);
    }
...
bzsim_free(BZSim *bz)
{
    WorkQueue *queue = bz->queue;
    queue->quit = True;
    while (queue->activeThreadsCount > 0) {
        mtx_lock(&queue->ordersRefillMtx);
        cnd_broadcast(&queue->ordersRefillCnd);
        mtx_unlock(&queue->ordersRefillMtx);
    }
    mtx_destroy(&queue->ordersRefillMtx);
    cnd_destroy(&queue->ordersRefillCnd);
    free(queue->orders);
    free(queue);
